...
## Every AudioPlayer usage must read `audio.readingLanguage`, not hardcode `.en`
`readingLanguage` (`context/AudioContext.tsx`, persisted key `@bibliaeN:readingLanguage`) is the single source of truth for which language's text gets sent to TTS, independent of the app's PT/EN display language (`LanguageContext`). Most surfaces (`chapter.tsx`, `daily.tsx`) correctly branch `readingLanguage === 'pt' ? item.pt : item.en` and fold it into their `queueKey` so switching language re-queues fresh text. One surface (the Home screen's daily-verse pill) was found hardcoding `verse.en` regardless of the setting — it silently ignored the user's Portuguese choice and looked like "audio is broken, only plays English" from the user's side, with no code error anywhere.

**Why:** This class of bug produces no error/log and passes casual testing (English audio "works fine") — it only surfaces when a real user has actually changed the language setting, which is easy for an agent to skip when spot-checking.

**How to apply:** When adding or auditing any `<AudioPlayer>`/`playQueue(...)` call site, confirm the item text is derived from `audio.readingLanguage` (not a bare `.en`/`.pt` field) and that the `queueKey` includes the language so a language switch actually creates a new queue instead of silently continuing the old one. Also consider surfacing an inline EN/PT toggle next to any player that lacks one — Settings' "Idioma do áudio" picker is easy to miss, so screens that support both languages should let users switch right where they're listening.
